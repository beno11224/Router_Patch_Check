...
##  Throughout this code I dont rename any of the fields of data, preferring to use list indecies for speed.
##  Below is a description of what each list item is:
##  item[0] = Hostname
##  item[1] = IP Address
##  item[2] = Patched?
##  item[3] = OS version
...
def main():
    if len(sys.argv) != 2:                      ##check if too many args
        ...
        data = remove_duplicates(parse_file(filename)) ##obtain 'data' from file
        data = get_updateables(data)            #remove unwanted data
        print_data_list(data)                   ##print final dataset
...
def parse_file(filename):
    ...
    return list of lists:fileContents
    '''
    file_contents = []
                                                ######open the file
    try:
    ...
    for line in f:
        ...
        for i in range(len(contents)):
            contents[i] = contents[i].strip()   ##to remove blank spaces from the strings so that same strings can be found    
                                                ##Here I would check for and remove headers, but that is not needed as tests
                                                ##on OS Version and Patched? lower down remove it anyway
        if len(contents) > 1:                   ##check that line is not 'blank'
            for item in file_contents:
                if item[0].lower() == contents[0].lower() or item[1].lower() == contents[1].lower():
                                                ##check whether the current item is a duplicate of another in the list (not case sensitive)
                    item.append("dup")          ##just append something onto the end of the item that needs to be deleted at the end (since
                                                ##valid data will only ever have 5 fields this allows for a simple <6 check later).
...
def get_updateables(data):
    ...
    for item in data:
        try:
            float(item[3])                      ##if the version cannot be turned into a float then ignore this bit of data as the version
                                                ##'number' will not be 'higher than 12'
        except:
            continue
        if (item[2].lower() == "no")and(float(item[3]) >= 12.0):
                                                ##check that router has not been updated and that
                                                ##OS Version is 12 or above
            data_list.append(item)
    return(data_list)

def print_data_list(data_list):
    '''
    pass in list of lists:dataList
